import {
	P as I,
	M as f,
	W as h,
	X as D,
	a as g,
	S as _,
	U as y,
	V as b,
	j as n,
	I as l,
	F as v,
	H as m,
	o as c,
	d as r,
	t as C,
	k,
	b as u,
} from "./vendor.c56b79f7.js";
const w = {
		name: "InsertImage",
		props: ["editor"],
		expose: ["openDialog"],
		data() {
			return { addImageDialog: { url: "", file: null, show: !1 } };
		},
		components: { Button: f, Dialog: h },
		methods: {
			openDialog() {
				this.addImageDialog.show = !0;
			},
			onImageSelect(t) {
				let e = t.target.files[0];
				!e ||
					((this.addImageDialog.file = e),
					D(e).then((i) => {
						this.addImageDialog.url = i;
					}));
			},
			addImage(t) {
				this.editor.chain().focus().setImage({ src: t }).run(), this.reset();
			},
			reset() {
				this.addImageDialog = this.$options.data().addImageDialog;
			},
		},
	},
	S = {
		class: "relative cursor-pointer rounded-lg bg-gray-100 py-1 focus-within:bg-gray-200 hover:bg-gray-200",
	},
	V = { class: "absolute inset-0 select-none px-2 py-1 text-base" },
	x = ["src"],
	B = u(" Insert Image "),
	N = u(" Cancel ");
function A(t, e, i, P, a, o) {
	const d = m("Button"),
		p = m("Dialog");
	return (
		c(),
		g(
			v,
			null,
			[
				_(t.$slots, "default", y(b({ onClick: o.openDialog }))),
				n(
					p,
					{
						options: { title: "Add Image" },
						modelValue: a.addImageDialog.show,
						"onUpdate:modelValue": e[2] || (e[2] = (s) => (a.addImageDialog.show = s)),
						onAfterLeave: o.reset,
					},
					{
						"body-content": l(() => [
							r("label", S, [
								r(
									"input",
									{
										type: "file",
										class: "w-full opacity-0",
										onChange:
											e[0] ||
											(e[0] = (...s) =>
												o.onImageSelect && o.onImageSelect(...s)),
										accept: "image/*",
									},
									null,
									32
								),
								r(
									"span",
									V,
									C(
										a.addImageDialog.file
											? "Select another image"
											: "Select an image"
									),
									1
								),
							]),
							a.addImageDialog.url
								? (c(),
								  g(
										"img",
										{
											key: 0,
											src: a.addImageDialog.url,
											class: "mt-2 w-full rounded-lg",
										},
										null,
										8,
										x
								  ))
								: k("", !0),
						]),
						actions: l(() => [
							n(
								d,
								{
									appearance: "primary",
									onClick:
										e[1] || (e[1] = (s) => o.addImage(a.addImageDialog.url)),
								},
								{ default: l(() => [B]), _: 1 }
							),
							n(d, { onClick: o.reset }, { default: l(() => [N]), _: 1 }, 8, [
								"onClick",
							]),
						]),
						_: 1,
					},
					8,
					["modelValue", "onAfterLeave"]
				),
			],
			64
		)
	);
}
var F = I(w, [["render", A]]);
export { F as default };
